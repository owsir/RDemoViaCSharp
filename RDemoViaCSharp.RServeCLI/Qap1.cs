...
namespace RserveCli
{
    ...
    internal class Qap1
    {
        ...
        /// </summary>
        /// <param name="socket">The this.socket through which we'll communicate with the server.</param>
        ...
        public List<byte> EncodeSexp(Sexp s)
        {
            ...
            if (s.Attributes.Count > 0)
            {
                ...
                res.AddRange(this.EncodeSexp(attrs));
            }
            ...
            else if (t == typeof(SexpTaggedList))
            {
                ...
                {
                    res.AddRange(this.EncodeSexp(a.Value));
                    res.AddRange(this.EncodeSexp(new SexpSymname(a.Key)));
                }
            }
            else if (t == typeof(SexpList))
            {
                ...
                foreach (var a in v)
                {
                    res.AddRange(this.EncodeSexp(a));
                }
            }
        ...
        public Sexp DecodeSexp(byte[] data, ref int start)
        {
            ...
            if ((xt & XtHasAttr) == XtHasAttr)
            {
                xt -= XtHasAttr;
                int oldstart = start;
                attrs = (SexpTaggedList)this.DecodeSexp(data, ref start);
            ...
            else
            {
                switch (xt)
                {
                    ...
                    case XtVector:
                        ...
                        while (start < end)
                        {
                            result.Add(this.DecodeSexp(data, ref start));
                        }
                        break;
                    case XtLangTag:
                    case XtListTag:
                        ...
                        while (start < end)
                        {
                            var val = this.DecodeSexp(data, ref start);
                            var key = this.DecodeSexp(data, ref start);
        ...
        public byte[] CommandReadStream(int cmd, IList<object> data)
        {
            ...
            {
                received += this.socket.Receive(res, received, toConsume - received, SocketFlags.None);
            }
                ...
                */
                ...
                switch (typ & 15) // only the lower 4 bits
                {
                    ...
                    case DtSexp:
                        {
                            int start = 0;
                            res.Add(this.DecodeSexp(dvbuf, ref start));
                        }
        ...
        private int SubmitCommand(int cmd, IList<object> data)
        {
            ...
            foreach (var a in data)
            {
                ...
                {
                    argbuf.AddRange(this.EncodeSexp(a as Sexp));
                    dt = DtSexp;
                }
                else if (a is byte[])
                {
            ...
            // Execute Command
            this.socket.Send(sbuf.ToArray());

            // Read Response
            var hdrbuf = new byte[16];
            if (this.socket.Receive(hdrbuf) != 16)
            {
                throw new WebException("Didn't receive a header.");
            }
            // && cmdResult != 2130771970
            int cmdResult = BitConverter.ToInt32(hdrbuf, 0);
            if ((cmdResult & 15) != 1)
            {
                throw new WebException("R threw an errors.");
            }
